...
**An example data file and a video-generating MATLAB script added:** The data file 'Example_data_over_time_WT_condition.dat' contains a large time trajectory of one WT simulation, generated using the FORTRAN code given here. The data structure is the same as described above. Additionally, the MATLAB script can be used to visualize the border cell migration over time using the data file. Notably, to run the MATLAB script, one needs to have MATLAB installed on the working desktop. The data file and the MATLAB script are to be kept in the same directory.
